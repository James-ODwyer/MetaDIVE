...
reads_threshold2=$8
log=$9
# as these are reads being plucked from read pairs the actual threshold of X reads needs to be half of the read threshold set as it will double back during extraction 
reads_threshold=$(($reads_threshold2 / 2))
...
run_kraken() {
    ...
    local kraken_cmd="kraken2 --db $krakendb --paired --threads $threads \
        --report ${krakenreport%.txt}_${output_suffix}.txt \
        --output ${krakenout%.txt}_${output_suffix}.txt $R1 $R2"
