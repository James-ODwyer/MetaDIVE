...
if (length(blastnfalsepospresence) ==0) {
  Blastnfalseposhits <- as.data.frame(matrix(nrow=0,ncol=18))
  paste0(NAMES," Blastnhits returned no findings")
}

colnames(Blastnfalseposhits) <- c("qseqid", "sseqid", "pident", "length", "evalue", "bitscore","staxids", "stitle", "qcovhsp", "multiplesp","staxidreduced", "superkingdom", "phylum", "class", "order", "family", "genus", "species")
...
for (i in c(1:nrow(Blastnfalseposhits))) {
  ...
  grep(pattern = Blastnfalseposhits$qseqid[i],x=allassignedfreqs$contig) -> idx
...
for (i in c(1:nrow(Blastnfalseposhits))) {
  ...
  grep(pattern = Blastnfalseposhits$qseqid[i],x=freqsummarynona$contig) -> idx
...
for (i in c(1:length(species_idvec))) {
  ...
  if (!is.na(contigssubset$percentident[1])) {
    ...
    contigsspecies[i,13] <- nrow(contigssubset)
  }
  
  if (sum(contigssubset$blastn_false_positive_check == "Yes")>=1){
    
    contigsspecies[i,14] <- "Yes"
    contigsspecies[i,15] <-(sum(contigssubset$blastn_false_positive_check == "Yes"))
    ...
    avg_idents <- contigssubset %>%
      filter(!is.na(blastn_alternate_percentident))
    ...
    avg_lengths <- contigssubset %>%
      filter(!is.na(blastn_alternate_alignment_length))
    
    avg_length_alt <- mean(as.numeric(avg_lengths$blastn_alternate_alignment_length))
    
    
    
    contigsspecies[i,16] <- top_countsuperkingdom
    contigsspecies[i,17] <- top_countspecies
    contigsspecies[i,18] <- avg_identity_alt
    contigsspecies[i,19] <- avg_length_alt
